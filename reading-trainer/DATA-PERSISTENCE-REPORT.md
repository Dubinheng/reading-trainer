...
- 数据库权限设置为 `600`。
- 存储账号密码哈希、会话哈希、邀请码、班级、学习设置、收藏、生词、错题、成绩、文章库，以及教师布置的题卡作业和每名学生的作业接收/提交记录。文章资源使用独立 `articles` 表，操作追踪使用追加式 `audit_events` 表；个人文章按 owner 隔离，系统文章按角色统一维护。
...
- 文章库和收藏可共用一张飞书表，通过 `数据类型` 区分；新文章使用 `articles:<article_id>` 稳定业务键。
...
- 管理员：管理账号、邀请码、班级、AI 配置、飞书同步和系统文章。
- 文章管理员：仅可登录后台并管理系统文章；不能访问账号、班级、学习数据、AI 或飞书功能。
...
4. 旧收藏和教师文章会转换为个人文章；服务器确认新增成功后才删除对应旧浏览器副本。服务器已有旧 `library/favorites` 文档在迁移期间以历史文章形式兼容读取。
5. 只有服务器写入成功后才删除对应旧浏览器副本；发生冲突或服务器失败时保留旧副本，不先删数据。
